...
| F-009 | Agent packs (loader + MCP + /pack run) | `[PARTIAL]` | `packages/agentpack/src/`, `packages/packs/*`, `apps/cli/src/commands/pack.ts`, [`packages/agentpack/docs/architecture.md`](../packages/agentpack/docs/architecture.md) | Phase 1: pack manifest (`pack.toml`), `loadPack()` with `extends` inheritance, `McpRegistry` (in-process + stdio MCP), `PackRuntime` chat tool-loop, mluxe `tools`/`tool_calls` support, `/pack list|info|run`. Example packs: `baseball-stats`, `astrologer`. 5 unit tests. Phases 2–4 (evals, widgets, diff evals, trajectory diffing) planned. |
